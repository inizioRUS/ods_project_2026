...
В рамках исследования проверялись две группы гипотез:

1. помогает ли доменная донастройка embedding-модели лучше находить юридически релевантные фрагменты;
2. улучшают ли hybrid retrieval, fusion и reranking качество ранних позиций выдачи по сравнению с отдельным dense/BM25-поиском.

### Датасеты и корпус

| Объект | Значение |
| ------ | -------- |
| Итоговый retrieval-корпус | примерно 1 200 чанков |
| Объём корпуса | примерно 92 000 символов / 55 000 токенов |
| Train triplets для fine-tuning | 1 992 |
| Уникальные positive chunks | 473 |
| Уникальные negative chunks | 456 |
| Уникальные документы в train/eval union | 583 |
| Eval-набор `legal-eval` | 111 запросов |
| Eval-корпус | 419 source chunks |
| Распределение eval-запросов | 49 ПДД, 36 КоАП, 26 УК РФ |

### Эффект fine-tuning embedding-модели

Сравнивалась базовая модель `intfloat/multilingual-e5-large-instruct` и дообученная версия на синтетических legal triplets. Донастройка улучшила все retrieval-метрики.

| Метрика | Baseline | Fine-tuned | Изменение |
| ------- | -------- | ---------- | --------- |
| Accuracy@1 / Recall@1 | 0.3063 | 0.5405 | +0.2342 |
| Accuracy@3 / Recall@3 | 0.5676 | 0.8288 | +0.2613 |
| Accuracy@5 / Recall@5 | 0.6577 | 0.8919 | +0.2342 |
| Accuracy@10 / Recall@10 | 0.7027 | 0.9279 | +0.2252 |
| MAP@10 | 0.4449 | 0.6910 | +0.2461 |
| MRR@10 | 0.4449 | 0.6910 | +0.2461 |
| nDCG@10 | 0.5083 | 0.7500 | +0.2416 |

Главный вывод: fine-tuning не просто добавляет правильный фрагмент где-то в top-k, а заметно поднимает его выше в ранжировании. Это видно по росту MRR@10 и nDCG@10.

### Архитектурные ablation-эксперименты

Сравнивались BM25, dense retrieval, hybrid fusion и reranking. Лучший результат дала конфигурация:
...
| Конфигурация | Recall@1 | Recall@5 | nDCG@1 | nDCG@5 |
| ------------ | -------- | -------- | ------ | ------ |
| BM25 only | 0.5676 | 0.7928 | 0.5676 | 0.7550 |
| multilingual-e5-large без BM25/rerank | 0.5676 | 0.8108 | 0.5676 | 0.7750 |
| multilingual-e5-large + BM25 + RRF | 0.6396 | 0.8468 | 0.6396 | 0.8150 |
| multilingual-e5-large + BM25 + weighted z-score | 0.6577 | 0.8649 | 0.6577 | 0.8402 |
| fine-tuned multilingual-e5-large без BM25/rerank | 0.6396 | 0.8649 | 0.6396 | 0.8310 |
| fine-tuned multilingual-e5-large + BM25 + weighted z-score | 0.7027 | 0.9009 | 0.7027 | 0.8721 |
| multilingual-e5-large + BM25 + weighted z-score + rerank | 0.8018 | 0.9550 | 0.8018 | 0.9672 |
| **fine-tuned multilingual-e5-large + BM25 + weighted z-score + rerank** | **0.8288** | **0.9730** | **0.8288** | **0.9695** |

### Основные выводы

* **Доменная донастройка эмбеддера полезна:** модель лучше различает близкие юридические нормы, например похожие штрафы, разные скоростные пороги или соседние части статей.
* **BM25 остаётся сильным baseline:** он хорошо работает с точными терминами, номерами статей, пунктами ПДД и числовыми условиями.
* **Hybrid retrieval лучше одиночного поиска:** dense retrieval покрывает перефразированные пользовательские вопросы, а BM25 ловит точные юридические формулировки.
* **Weighted z-score fusion оказался стабильнее RRF и weighted sum:** z-score нормализация уменьшает проблему разных шкал dense-score и BM25-score.
* **Reranking даёт самый сильный финальный прирост:** cross-encoder заново оценивает пары `query — chunk` и поднимает наверх юридически точные фрагменты.
* **Лучшая система достигла 0.8288 Recall@1 и 0.9730 Recall@5**, то есть почти всегда передаёт генератору правильный источник в верхних результатах.
